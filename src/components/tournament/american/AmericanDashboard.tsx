...
import { Flag, Loader2, Share2, Clock, PlayCircle, Trophy, LineChart } from 'lucide-react'
import { Tabs, TabsContent, TabsList, TabsTrigger } from '@/components/ui/tabs'
import { Button } from '@/components/ui/button'
import { AmericanEvolutionChart, type EvolutionSeries } from './AmericanEvolutionChart'
import { EncounterIndicator } from './EncounterIndicator'
...
import { RoundsMatchCard } from '@/components/tournament/rounds/RoundsMatchCard'
import { StandingsTable } from '@/components/tournament/StandingsTable'
import {
  closeRound,
  startNextAmericanRound,
  finishAmericanTournament,
  submitAmericanScore,
  resetAmericanScore,
} from '@/app/(app)/tournaments/[id]/actions'
...
export interface AmericanMatchInfo {
  id: string
  status: string
...
export interface AmericanByePlayer {
  playerId: string
  playerName: string
}

interface AmericanDashboardProps {
  tournamentId: string
  tournamentSlug: string
  tournamentStatus: string
  currentRound: { id: string; round_number: number; status: string } | null
  completedRounds: number
  matches: AmericanMatchInfo[]
  byePlayers: AmericanByePlayer[]
  standings: AmericanStandingRow[]
  setsToWin: 1 | 2
  targetScore: number
  minGap: number
  evolution: { rounds: number[]; series: EvolutionSeries[] }
  encounters: { pairs: EncounterPair[]; repeatedPairs: number; maxRepeat: number }
  playerNames: Record<string, string>
}

function groupByWave(matches: AmericanMatchInfo[]): Map<number, AmericanMatchInfo[]> {
  const map = new Map<number, AmericanMatchInfo[]>()
...
function isWaveDone(waveMatches: AmericanMatchInfo[]): boolean {
  return waveMatches.every((m) => m.status === 'done' || m.status === 'bye')
}

export function AmericanDashboard({
  tournamentId,
  tournamentSlug,
  tournamentStatus,
  currentRound,
  ...
  function renderMatch(match: AmericanMatchInfo, waveBlocked: boolean) {
    return (
      <RoundsMatchCard
        key={match.id}
        matchId={match.id}
        ...
        minGap={minGap}
        waveBlocked={waveBlocked}
        submitAction={submitAmericanScore}
        resetAction={resetAmericanScore}
    ...
    <div className="space-y-4">
      ...
      {matches.length === 0 ? (
        <div className="bg-surface border border-subtle rounded-xl p-8 text-center text-muted text-sm">
          {currentRound ? 'Aucun match pour ce round.' : 'Le premier round sera généré au démarrage.'}
        </div>
      ) : hasMultipleWaves ? (
          ...
          {waveNumbers.map((wn) => {
            ...
            return (
              <TabsContent key={wn} value={String(wn)} className="space-y-2">
                {waveMatches.map((match) => renderMatch(match, !prevDone))}
              </TabsContent>
            )
          })}
        </Tabs>
      ) : (
        <div className="space-y-2">{matches.map((match) => renderMatch(match, false))}</div>
      )}

      {/* Joueurs en attente ce round */}
      {byePlayers.length > 0 && (
        <div className="bg-surface border border-subtle rounded-xl p-4 space-y-2">
          <h3 className="text-muted text-xs font-medium uppercase tracking-wide">En attente ce round</h3>
          {byePlayers.map((p) => (
  ...
  // ─── Statistiques : évolution + rencontres ───────────────────────────────────

  const statsSection = (
    <div className="bg-surface border border-subtle rounded-xl p-4">
      <div className="flex items-center gap-2 mb-4">
        <LineChart className="w-4 h-4 text-primary" />
        <h2 className="font-display font-bold text-white">Statistiques</h2>
      </div>
      <Tabs defaultValue="evolution">
        <TabsList className="w-full bg-surface-alt border border-subtle mb-4">
          <TabsTrigger value="evolution" className="flex-1 data-[state=active]:bg-primary data-[state=active]:text-app">
            Évolution
          </TabsTrigger>
          <TabsTrigger value="encounters" className="flex-1 data-[state=active]:bg-primary data-[state=active]:text-app">
            Rencontres
          </TabsTrigger>
        </TabsList>
        <TabsContent value="evolution">
          <AmericanEvolutionChart rounds={evolution.rounds} series={evolution.series} />
        </TabsContent>
        <TabsContent value="encounters">
          <EncounterIndicator
            pairs={encounters.pairs}
            repeatedPairs={encounters.repeatedPairs}
            maxRepeat={encounters.maxRepeat}
            nameOf={playerNames}
          />
        </TabsContent>
      </Tabs>
  ...
  return (
    <>
      {/* Desktop : 2 colonnes + bandeau stats pleine largeur */}
      <div className="hidden lg:block lg:space-y-6">
        <div className="grid grid-cols-3 gap-6">
          <div className="col-span-2">{matchesSection}</div>
          <div className="col-span-1">{standingsSection}</div>
        </div>
        {statsSection}
      </div>
      ...
      <div className="lg:hidden">
        <Tabs defaultValue="matches">
          <TabsList className="w-full bg-surface border border-subtle mb-4">
            ...
            <TabsTrigger value="stats" className="flex-1 data-[state=active]:bg-primary data-[state=active]:text-app">
              Stats
            </TabsTrigger>
          </TabsList>
        ...
        <AlertDialogContent className="bg-surface border-subtle">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-muted">
              Le classement sera figé pour ce round. Tu pourras ensuite lancer le round suivant ou terminer le tournoi.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
        ...
        <AlertDialogContent className="bg-surface border-subtle">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-muted">
              ...
              Le classement actuel devient le classement final. Cette action est irréversible.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel className="border-subtle text-muted hover:text-white hover:bg-surface-alt">
